...
const int wxEVT_LINEARCONTROLLER_FIRST = wxEVT_FIRST + 5401;

const wxEventType wxEVT_LINEARCONTROLLER	= wxEVT_LINEARCONTROLLER_FIRST + 1;

#define EVT_LINEARCONTROLLER(id, fn)	\
		DECLARE_EVENT_TABLE_ENTRY(wxEVT_LINEARCONTROLLER, id, -1, \
		(wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction)&fn, \
		(wxObject*)NULL ),

class WXDLLIMPEXP_CORE wxSimpleLinearController : public wxControl
{
public:
	wxSimpleLinearController(void) { m_id = -1 ; };

	wxSimpleLinearController(wxWindow *parent,
				const wxWindowID id         = -1,
				const wxPoint&   pos        = wxDefaultPosition,
				const wxSize&    size       = wxDefaultSize,
				const long int   style      = 0);

	virtual ~wxSimpleLinearController();

	void SetRangeVal(int min, int max) {m_nMin = min; m_nMax = max ; } ;
	void SetHorizontalDirection(bool dir) { m_bDirHorizFlag = dir ; } ;
...
private:
	...
	void DrawCurrentValue (wxDC &dc);  // Print numeric value in middle of the slider
